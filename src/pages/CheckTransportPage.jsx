...
<<<<<<< HEAD
  ...
  // ── Pending   = actual3 (col X) IS empty AND order is staged
  // ── Completed = actual3 (col X) NOT empty
  const isStaged = (row) => hasValue(row.planned3) || hasValue(row.actual3) || hasValue(row.actual2) || hasValue(row.planned2) || hasValue(row.actual1);
  const isPending = (row) => isStaged(row) && !hasValue(row.actual3);
  const isCompleted = (row) => hasValue(row.actual3);
    ...
    const updated = fmsData.map((r) =>
      r.poNumber === item.poNumber
        ? {
          ...
          'Delivery location': editLocation,
          'Delivery Location': editLocation,
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-3 text-muted-foreground">
                        ...
                        <p className="text-sm font-semibold text-foreground/70">Loading transport data...</p>
                      </div>
                    </TableCell>
                  </TableRow>
...
<<<<<<< HEAD
                                  ...
                                  const currentTransporter = item.transporterName || item['Transporter name'] || item['Transporter Name'] || item['Transporter'] || '';
                                  setTransporterName(currentTransporter);
                                  setVehicleNumber(item.vehicleNumber || item['Vehicle Number'] || item['Vehicle number'] || '');
...
<<<<<<< HEAD
                ...
                { label: 'Vehicle Number', value: detailDialog.item.vehicleNumber || detailDialog.item['Vehicle Number'] || '—' },
